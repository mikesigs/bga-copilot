...
- [Onboarding/settings UX for the LLM API key](tickets/015-api-key-onboarding-ux.md) — confirmed with user: settings live in-panel via a header icon (third in-panel view, not an external options page); Chat view blocks with an inline "add your API key" prompt until one is saved; key validated with a test call before saving; plain one-sentence cleartext-storage disclosure shown in the settings view; both providers' keys persist independently so switching back and forth doesn't require re-entry.
- [Error/fallback handling for rulebook and state-extraction failures](tickets/014-error-fallback-handling.md) — never refuse to chat: degrade and surface a small non-blocking inline callout (reusing the ticket 009 rulebook-callout pattern) for every failure mode. Rulebook failures fall back to state-only/general-knowledge context. State extraction retries briefly, then degrades per-state (plugin throws → generic-core summary for that state) or fully (no `gameui` found → pure chat, clearly noted) — no blocking modals or banners.
- [Validation/testing strategy for extending the generic engine to new games](tickets/016-testing-strategy-new-games.md) — manual-only discovery (open devtools, diff `gamedatas` keys against the generic-core list) since BGA has no test/sandbox API; partial per-state plugin coverage is an acceptable shipping bar (unhandled states degrade to generic, per ticket 014); a minimal fixture-replay script (saved `gamedatas` snapshots run through `summarizeState`) is worth it as a cheap regression check, no test framework needed.
- [Rate limiting and cost controls for cloud LLM usage](tickets/017-rate-limiting-cost-controls.md) — minimal guardrails only: a short send-debounce as a bug guard (not a real rate limiter); no cost/usage-estimate UI (the provider dashboard already covers this); a pre-send size-threshold warning reusing ticket 007's existing token-budget math; no hard spending cap (provider-side limits cover it).
- [Handling multiple simultaneous BGA game tabs/tables](tickets/018-multiple-tabs-handling.md) — confirms existing decisions already cover this: per-tab isolation (ticket 004) plus per-table-id storage (ticket 006) handle different tables in different tabs for free; same-table-in-two-tabs races are accepted as a rare, low-stakes edge case rather than solved; no service-worker queuing needed at personal-use scale.
